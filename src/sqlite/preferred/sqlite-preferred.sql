...
CREATE TABLE application (
	...
	CHECK (key is not null or LENGTH(key) = 3 ),
	CHECK (name is null or LENGTH(name) >= 5 )
 );
...
CREATE TABLE log ( 
	...
	CHECK ( entry_type IN ('INFO', 'WARNING', 'ERROR', 'FATAL', 'DEBUG') )
 );
...
CREATE TABLE version ( 
	...
	comments             text     ,
	CHECK ( status in ('STARTED', 'RUNNING', 'ENDED', 'FAILED') )
 );
